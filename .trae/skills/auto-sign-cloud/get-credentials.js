...
function resolveProfileRoot(names, requiredFiles) {
  ...
  return path.join(APPDATA, names[0]);
}
...
const TRAE_STORAGE = path.join(TRAE_ROOT, 'User', 'globalStorage', 'storage.json');
const CB_ROOT = resolveProfileRoot(
  ['CodeBuddy CN', 'WorkBuddy CN', 'WorkBuddy', 'CodeBuddy'],
  [
    path.join('User', 'globalStorage', 'storage.json'),
...
const CB_STORAGE = path.join(CB_ROOT, 'User', 'globalStorage', 'storage.json');
const CB_VSCDB = path.join(CB_ROOT, 'User', 'globalStorage', 'state.vscdb');
const CB_LOCAL_STATE = path.join(CB_ROOT, 'Local State');
...
function readTrae() {
  console.log('[Trae] 使用数据目录:', TRAE_ROOT);
  ...
  const localEnvPath = path.join(TRAE_ROOT, 'ModularData', 'ckg_server', 'local_env.json');
...
// ============================================================
const QODER_ROOT = resolveProfileRoot(
  ['QoderCN', 'Qoder CN', 'Qoder'],
  [
    path.join('User', 'globalStorage', 'state.vscdb'),
    'Local State'
  ]
);
const QODER_VSCDB = path.join(QODER_ROOT, 'User', 'globalStorage', 'state.vscdb');
const QODER_LOCAL_STATE = path.join(QODER_ROOT, 'Local State');
...
function readWorkBuddy() {
  console.log('[WorkBuddy] 使用数据目录:', CB_ROOT);
...
function readQoder() {
  console.log('[Qoder] 使用数据目录:', QODER_ROOT);
...
function resultMessage(r) {
  if (r.error) return `网络错误: ${r.error}`;
  return r.json ? JSON.stringify(r.json) : (r.raw || `HTTP ${r.http}`);
}

async function signInTrae(c) {
  if (!c.accessToken || !c.deviceId) throw new Error('缺少 accessToken 或 deviceId');
  const base = (c.host || 'https://api.trae.cn').replace(/`/g, '').trim();
  const headers = {
    'Authorization': `Cloud-IDE-JWT ${c.accessToken}`,
    'Content-Type': 'application/json',
    'x-device-id': c.deviceId
  };
  if (c.userRegion) headers['X-User-Region'] = c.userRegion;
  const body = { req_source: 1 };

  const status = await postJson(`${base}/trae/api/v2/ug/checkin_credits/status`, headers, body);
  if (status.error) return `签到失败: ${status.error}`;
  if (status.json?.code !== 0) return `签到失败: ${resultMessage(status)}`;
  if (status.json.checked_in === true || status.json.did_checked_in === true) return '今日已签到';

  const delays = [2000, 5000, 10000];
  for (let i = 0; i < delays.length; i++) {
    const claim = await postJson(`${base}/trae/api/v2/ug/checkin_credits/claim`, headers, body);
    if (claim.json?.code === 0) {
      const points = claim.json.data?.points || claim.json.points || 0;
      return `签到成功 +${points} 积分`;
    }
    if (claim.json?.code === 1001) return '今日已签到';
    if (claim.json?.code !== 9074) return `签到失败: ${resultMessage(claim)}`;
    if (i < delays.length - 1) await new Promise(resolve => setTimeout(resolve, delays[i]));
  }
  return '签到失败: 9074 限流,重试后仍未成功';
}

async function signInWorkBuddy(c) {
  if (!c.accessToken || !c.uid) throw new Error('缺少 accessToken 或 uid');
  ...
  const status = await postJson(`${base}/v2/billing/meter/checkin-activity-status`, headers, {});
  if (status.error) return `签到失败: ${status.error}`;
  if (status.json?.code !== 0) return `签到失败: ${resultMessage(status)}`;
  if (status.json.data?.today_checked_in === true) return '今日已签到';

  const claim = await postJson(`${base}/v2/billing/meter/daily-checkin`, headers, {});
  if (claim.json?.code === 0) {
    const credit = claim.json.data?.credit || 0;
    const streak = claim.json.data?.streak_days || 0;
    return `签到成功 +${credit} 积分, 连签${streak}天`;
  }
  if (claim.json?.code === 10001) return '今日已签到';
  return `签到失败: ${resultMessage(claim)}`;
}

async function signInQoder(c) {
  if (!c.accessToken) throw new Error('缺少 accessToken');
  ...
  const list = await getJson(`${base}/sash/api/v1/me/campaigns`, headers);
  if (list.error) return `签到失败: ${list.error}`;
  if (!list.json || list.http !== 200) return `签到失败: ${resultMessage(list)}`;
  if (list.json.claimable !== true) return '今日无待领取活动或活动未开启';

  const campaigns = (list.json.campaigns || []).filter(camp =>
    camp && camp.actionType === 'CLAIM_BENEFIT' && camp.claimStatus === 'CLAIMABLE'
  );
  if (campaigns.length === 0) return '今日已签到';

  let claimed = 0;
  let totalCredit = 0;
  for (const campaign of campaigns) {
    const claim = await postJson(
      `${base}/sash/api/v1/me/campaigns/${campaign.campaignId}/claim`,
      Object.assign({}, headers, { 'Content-Type': 'application/json' }),
      {}
    );
    if (claim.json?.status === 'CLAIMED') {
      claimed++;
      totalCredit += campaign.benefit?.amount || 0;
    } else if (claim.json?.replayed !== true) {
      return `签到失败: ${resultMessage(claim)}`;
    }
  }
  return claimed > 0 ? `签到成功 +${totalCredit} Credits, 共${claimed}项` : '今日已签到';
}
...
(async () => {
  ...
  out('================ 开始执行三平台签到 ================');
  const signIns = [
    ['Trae', trae, signInTrae],
    ['WorkBuddy / CodeBuddy', wb, signInWorkBuddy],
    ['Qoder CN', qoder, signInQoder]
  ];
  for (const [name, credential, signIn] of signIns) {
    if (!credential) {
      out(`【${name}】跳过: 未读取到凭证`);
      continue;
    }
    try {
      out(`【${name}】${await signIn(credential)}`);
    } catch (e) {
      out(`【${name}】签到失败: ${e.message}`);
    }
  }
